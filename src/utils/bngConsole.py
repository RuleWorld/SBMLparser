...
class BNGConsole:
    def __init__(self,bngExecutable):
        self.bngExecutable = bngExecutable

    def getBngExecutable():
        return self.bngExecutable

    def bngl2xml(self, bnglFile, timeout=60):
        try:
            bngconsole = pexpect.spawn('{0} --console'.format(self.getBngExecutable()), timeout=timeout)
    ...
    def bngl2sbml(self, bnglFile, timeout=60):
        try:
            bngconsole = pexpect.spawn('{0} --console'.format(self.getBngExecutable()), timeout=timeout)
            bngconsole.expect('BNG>')
            bngconsole.sendline('load {0}'.format(bnglFile))
    ...
    def writeNetwork(self, bnglFile):
        bngconsole = pexpect.spawn('{0} --console'.format(getBngExecutable()))
        bngconsole.expect('BNG>')
        bngconsole.sendline('load {0}'.format(bnglFile))
        bngconsole.expect('BNG>')
    ...
    def generateGraph(self, bnglFile,graphType,options=[]):
